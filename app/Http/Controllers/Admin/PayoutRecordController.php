...
class PayoutRecordController extends Controller
{
    ...
    public function reportSearch(Request $request)
    {
        ...
        $fund_sum = (float) number_format((clone $baseQuery)->sum('amount'), 2, '.', '');
        ...
        try {
            ...
            if ($request->status == '2') {
                ...
                if ($data->status == "Complete") {
                    ...
                    $log_exist = Log::where('transection_id', $data->id)->where('transection_type', 2)
                        ->orderBy('id', 'desc')
                        ->first();

                    if (isset($log_exist)) {
                        DB::rollBack();
                        throw new \Exception("This transaction log already exist.");
                    }
    ...
    public function workboard(Request $request)
    {
        ...
        foreach ($apis as $api) {
            ...
            $successRate = $data->total_received > 0
                ? (float) number_format((($data->total_processed / $data->total_received) * 100), 2, '.', '')
                : 0;
        ...
        $pageTitle = "Workboard";
        // $apis = Api::get();
        return view('admin.payout.workboard', compact(
            'pageTitle',
            'apis',
            'highPerformance',
            'mediumPerformance',
            ...
            if ($request->status == 'Complete') {
                ...
                if ($partner_api_key && !empty($partner_api_key->api_endpoint_deposit) && $partner_api_key->website != env('APP_WEBSITE')) {
                    ...
                    $array_data = [
                        ...
                        'e_wallet_name' => $data->gateway->name,
                        'amount' => $this->convertStringToNumber($data->amount),
                        'request_amount' => $this->convertStringToNumber($data->request_amount),
