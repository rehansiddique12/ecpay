...
h1, h2, h3, h4, h5 {
    ...
    <style>
        ...
        .bkash-responsive-row {
            /* height: 100px; */
            background-image: url('<?php echo e(asset('assets/images/bKash_Background.jpg')); ?>');
        ...
        .nagad-responsive-row {
            /* height: 100px; */
            background-image: url('<?php echo e(asset('assets/images/Nsagad_backgroudn.jpg')); ?>');
            background-size: 100% 100%;
            background-repeat: no-repeat;
            margin: 0 -15px;
            /* Adjusts the margin to fit within the card */
        ...
        .rocket-responsive-row {
            /* height: 100px; */
            background-image: url('<?php echo e(asset('assets/images/Rocket_Background.jpg')); ?>');
            background-size: 100% 100%;
            background-repeat: no-repeat;
            margin: 0 -15px;
            /* Adjusts the margin to fit within the card */
            ...
            <div class="payment-container">
                <div id="intime">
                    <div style="min-height:70px" class="">
                        <img src="<?php echo e($logo); ?>" style="width: 50%;" alt="Logo">
                        <span style="float: right;margin-top:20px" id="timer" class="<?php echo e($time_class); ?>">00:00</span>
                    ...
                    <div style="background-color: black;text-align:center;" class="pb-1">
                        ...
                        <p class="amount" style="color: #ffc000"><?php echo e(number_format($data['amount'], 2, '.', ',')); ?> Tk</p>
                    ...
                    <div style="background-color: <?=$bgcolorrbga?>;border-radius:5px" class="mt-2 mb-2 ml-3 mr-3 p-2">
                        <div style="display: flex; align-items: center;">
                            ...
                            <div>
                                <span style="font-weight: bold">STEP 1: Copy the <span id="acctype1"><?php echo e($data['account_type']); ?></span> Account <br> ধাপ ১: এজেন্ট অ্যাকাউন্টটি কপি করুন</span><br>
                            </div>
                        </div>


                        <div style="background-color: white;border-radius:2px" class="p-2">
                            <span style="font-weight: bold;color:<?=$bgcolor?>" id="acctype2"><?php echo e($data['account_type']); ?></span> <span id="accountNumber"> <?php echo e($data['phone_number']); ?></span>
                        ...
                        <div style="background-color: <?=$bgcolor?>;text-align:center;" class="p-2 mt-2">
                            <span style="font-weight: bold;color:white">This <?php echo e($ewallet_to_show); ?> number only accept cashout.<br>
                            এই <?php echo e($bangla_ewallet); ?> নম্বরটি শুধুমাত্র ক্যাশআউট গ্রহণ করে।
                            </span>
                        </div>
                        ...
                        <form action="<?php echo e(route('iframe.payment2')); ?>" method="POST" id="payment-form">
                            <?php echo csrf_field(); ?>
                        <div class="input-container" style="position: relative;">
                            <input type="text" name="txn" class="form-control" id="txn_verification"
                                ...
                                data-min="<?php echo e(($ewallet == 'bkash') ? 10 : (($ewallet == 'nagad') ? 8 : 1)); ?>"
                        ...
                        <small id="txn_error" style="color: red; display: none;"></small>
                            ...
                            <input type="hidden" name="ewallet" value="<?php echo e($data['ewallet']); ?>">
                            <input type="hidden" name="amount" value="<?php echo e($data['amount']); ?>">
                            <input type="hidden" id="fund_id" name="fund_id" value="<?php echo e($data['gate_id']); ?>">
                        ...
                        <?php if(!empty($data['redirect_url'])): ?>
                        <a href="<?php echo e($data['redirect_url']); ?>" class="form-control btn custombtn-outline mt-2 mb-2">Return to the merchant</a>
                        <?php endif; ?>
...
$('#complete-button').on('click', function(event) {
    ...
    <script src="<?php echo e(asset('assets/iframe/clipboard.min.js')); ?>"></script>
    <script>
    $(document).ready(function() {
...
<script>
    ...
    function checkMinLength() {
        ...
        if (ewalletType === 'bkash' && !/^[A-Za-z]/.test(txnValue)) {
            errorMsg.innerText = '<?php echo e($ewallet_to_show); ?> Transaction ID must start with a letter (A-Z).';
            errorMsg.style.display = 'block';
            return;
        }

        if (ewalletType === 'nagad' && !/^[0-9]/.test(txnValue)) {
            errorMsg.innerText = '<?php echo e($ewallet_to_show); ?> Transaction ID must start with a digit (0-9).';
            errorMsg.style.display = 'block';
            return;
        }
    }
...
<script>
        $(document).ready(function() {
            ...
            $.ajax({
                url: "<?php echo e(route('iframe.getaccount')); ?>", // Define the route for POST request
                type: 'POST',
                dataType: 'json',
                data: {
                    _token: "<?php echo e(csrf_token()); ?>", // CSRF token for security
                    ewallet: "<?php echo e($ewallet); ?>",
                    amount: "<?php echo e($amount); ?>",
                    username: "<?php echo e($data['username']); ?>",
                    acc: "<?php echo e($data['acc']); ?>",
                    transection_id: "<?php echo e($data['transection_id']); ?>",
                    member_id: "<?php echo e($data['member_id']); ?>",
                    gate_id: "<?php echo e($data['gate_id']); ?>"
                },
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\ecpay\resources\views/partner/payout/process_transection2.blade.php ENDPATH**/ ?>