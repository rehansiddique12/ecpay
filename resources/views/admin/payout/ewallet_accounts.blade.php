<x-admin-layout :title="$pageTitle">
                    ...
                    <div class="table-responsive">
                        <table class="categories-show-table table table-hover table-striped table-bordered">
                            <thead class="thead-dark">
                                <tr>
                                    <th scope="col">{{ __('accounts.e_wallet_account_no') }}</th>
                                    <th scope="col">{{ __('accounts.e_wallet_name') }}</th>
                                    <th scope="col">{{ __('accounts.type') }}</th>
                                    <th scope="col">{{ __('accounts.live_status') }}</th>
                            ...
                            <tbody>
                                @forelse($records as $key => $item)
                                    <tr>
                                        ...
                                        <td>
                                            <span id="status-indicator-{{ $item['id'] }}"
                                                class="{{ $item['live'] == 1 ? 'dot' : 'reddot' }}"></span>
                                        </td>
                                        <td data-label="@lang('Status')" class="text-lg-center text-right">
                                            @if ($item->status == 1)
                                                <span class="badge badge-light"> <i
                                                        class="icon-base ti tabler-IconCircleFilled me-1 text-success"
                                                        style="border-radius: 40px;"></i>
                                                    {{ __('accounts.active') }}</span>
                                            @else
                                                <span class="badge badge-light"> <i
                                                        class="icon-base ti tabler-IconCircleFilled text-danger me-1 text-success"
                                                        style="border-radius: 40px;"></i>
                                                    {{ __('accounts.inactive') }}</span>
                                            @endif
                                        </td>
                                                ...
                                                <div class="dropdown-menu">
                                                    ...
                                                    @if (adminAccessRoute(config('role.e_wallet_accounts_test.access.edit')))
                                                        <button type="button" class="btn btn-sm edit_button"
                                                            ...
                                                            onclick="setBalanceItem({{ $item['id'] }}, '{{ $item['e_wallet_name'] }}')">
                                                            <i
                                                                class="icon-base ti tabler-cash-banknote me-1 text-success"></i>
                                                            {{ __('accounts.test_account') }}
                                                        </button><br>
                                                        <a href="{{ route('admin.e_wallet_accounts.toggle_status', $item['id']) }}"
                                                            class="btn btn-sm">
                                                            @if ($item->status == 1)
                                                                <i
                                                                    class="icon-base ti tabler-circle-dashed-x me-1 text-danger"></i>
                                                                @lang('')
                                                                {{ __('accounts.deactivate') }}
                                                            @else
                                                                <i
                                                                    class="icon-base ti tabler-check me-1 text-success"></i>
                                                                {{ __('accounts.activate') }}
                                                            @endif
                                                        </a>
                                                    @endif
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card card-primary m-0 m-md-4 my-4 m-md-0 shadow">
                <div class="card-body">
                    <h3>{{ __('accounts.admin_e_wallet_testing_accounts') }}</h3>
                    @if (adminAccessRoute(config('role.e_wallet_accounts_test.access.add')))
                        <button type="button" class="btn btn-success btn-sm edit_button mb-3" data-bs-toggle="modal"
                            data-bs-target="#newModalc">
                            {{ __('accounts.add_admin_account') }}
                        </button>
                    @endif
                    <div class="table-responsive">
                        <table class="categories-show-table table table-hover table-striped table-bordered">
                            <thead class="thead-dark">
                                <tr>
                                    <th scope="col">{{ __('accounts.e_wallet_account_no') }}</th>
                                    <th scope="col">{{ __('accounts.e_wallet_name') }}</th>
                                    <th scope="col">{{ __('accounts.action') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td data-label="@lang('Action')">
                                            @if (adminAccessRoute(config('role.e_wallet_accounts_test.access.delete')))
                                                <a href="#" class="btn btn-sm edit_button text-danger delete-role"
                                                    data-id="{{ $item['id'] }}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="modalTopTitle">{{ __('accounts.ewallet_test') }}</h5>
                        ...
                        <div class="row justify-content-between align-items-center">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label class="pr-3">{{ __('accounts.admin_account_no') }}</label>
                                    <select required name="account_no" id="acc_no" class="form-control"></select>
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label class="pr-3">{{ __('accounts.amount') }}</label>
                                    <input type="text" value="200" class="form-control" name="amount"
                                        readonly />
                                </div>
                            </div>
                            <div class="col-md-12">
                                <button type="button" id="runWithdrawalTest" class="btn btn-primary mt-3">
                                    {{ __('accounts.run_test') }}
                                </button>
                                <br><br>
                                {{ __('accounts.withdrawal') }}
    ...
    <div class="modal modal-top fade" id="newModalc" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="modalTopTitle">{{ __('accounts.add_admin_account') }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('admin.ewallet.accounts.add') }}" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="row justify-content-between align-items-center">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label class="pr-3">{{ __('accounts.account_no') }}</label>
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    <input id="Nagad" value="Nagad" type="radio" checked
                                        name="e_wallet_name" />
                                    <label for="Nagad" class="pr-3">{{ __('accounts.nagad') }}</label>
                                    <br>
                                    <input id="Rocket" value="Rocket" type="radio" name="e_wallet_name" />
                                    <label for="Rocket" class="pr-3">{{ __('accounts.rocket') }}</label>
                                    <br>
                                    <input id="bKash" value="bKash" type="radio" name="e_wallet_name" />
                                    <label for="bKash" class="pr-3">{{ __('accounts.bkash') }}</label>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="submit" class="btn btn-primary">{{ __('accounts.add') }}</button>
    ...
    @push('js')
        ...
        <script>
            $(document).on('click', '.delete-role', function(e) {
                ...
                // SweetAlert2 confirmation dialog with localization
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            error: function(xhr, status, error) {
                                Swal.fire(
                                    '{{ __('accounts.error_title') }}',
                                    '{{ __('accounts.error_message') }}',
                                    'error'
                                );
                            }
                        });
