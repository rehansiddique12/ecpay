    ...
    <div class="page-header card card-primary m-0 m-md-4 my-4 m-md-0 p-5 shadow">
        <form action="{{ route('admin.api.post.commissions') }}" method="get">
            ...
            <div class="row justify-content-between align-items-center">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        <label>{{ __('partner.parent') }}</label>
                        <select name="parent" class="form-select select2" data-allow-clear="true"
                            data-placeholder="{{ __('partner.select_partner') }}">
                            <option disabled selected></option>
                            <option value="" @if (request()->parent === '') selected @endif>
                                {{ __('partner.all') }}</option>
                            @foreach ($partners as $partner)
                                <option value="{{ $partner->id }}" @selected(request()->parent == $partner->id)>{{ $partner->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <a href="{{ route('admin.commissions.export', request()->all()) }}"
                            ...
                            <i class="icon-base ti tabler-download me-1"></i> {{ __('partner.export') }}
                        </a>
                    </div>
                </div>
                    ...
                    <div class="table-responsive">
                        <table class="categories-show-table table table-hover table-striped table-bordered">
                            <thead class="thead-dark">
                                <tr>
                                    <th scope="col">{{ __('partner.partner_agent') }}</th>
                                    <th scope="col">{{ __('partner.type') }}</th>
                                    <th scope="col">{{ __('partner.amount') }}</th>
                                    <th scope="col">{{ __('partner.charges') }}</th>
                                    <th scope="col">{{ __('partner.net_amount') }}</th>
                                    <th scope="col">{{ __('partner.profit') }}</th>
                                    <th scope="col">{{ __('partner.parent') }}</th>
                                        ...
                                        <td data-label="{{ __('partner.status') }}" class="text-lg-center text-right">
                                            @if ($item->type == 2)
                                                <span class="badge bg-danger">
                                                    <i class="fa fa-circle  text-white danger font-12"></i>
                                                    {{ __('partner.withdrawal') }}
                                                </span>
                                            @elseif($item->type == 1)
                                                <span class="badge bg-success">
                                                    <i class="fa fa-circle text-white success font-12"></i>
                                                    {{ __('partner.deposit') }}
                                                </span>
                                            @endif
                                        </td>
                                ...
                                @if ($isLastPage && $totalAmount)
                                    <tr>
                                        <td colspan="2" class="text-right">
                                            <strong>{{ __('partner.total') }}:</strong></td>
