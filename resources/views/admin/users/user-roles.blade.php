                        ...
                        @if (adminAccessRoute(config('role.roles_category.access.add')))
                            <div class="d-flex gap-2 mb-3">
                                <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                    ...
                                    {{ __('userManagement.add_new_role') }}
                                </button>
                                <button type="button" class="btn btn-warning" id="openCloneModal"
                                    data-bs-toggle="modal" data-bs-target="#cloneModal">
                                    {{ __('userManagement.clone_role_permission') }}
                                </button>
                            </div>
                        @endif
                        <div class="table-responsive">
                            <table id="rolesTables"
                                ...
                                <thead class="thead-dark">
                                    <tr>
                                        <th scope="col">{{ __('userManagement.no') }}</th>
                                        <th scope="col">{{ __('userManagement.roles_name') }}</th>
                                        <th class="text-center" scope="col">{{ __('userManagement.action') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
            ...
            <div class="modal-content">
                <form id="addRolesForm">
                    ...
                    <div class="modal-header">
                        <h5 class="modal-title" id="newModalLabel">{{ __('userManagement.add_role') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                            aria-label="{{ __('userManagement.close') }}"></button>
                    </div>
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="roles_name" class="form-label">{{ __('userManagement.role_name') }}</label>
                            <input type="text" class="form-control" id="roles_name" name="role"
                                placeholder="{{ __('userManagement.enter_role_name') }}" required>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary"
                            id="saveRoleBtn">{{ __('userManagement.save_role') }}</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="addRolesFormCopy" action="{{ route('admin.roles.copy') }}" method="POST">
                    ...
                    <div class="modal-header">
                        <h5 class="modal-title" id="cloneModalLabel">{{ __('userManagement.clone_role_permission') }}
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                            aria-label="{{ __('userManagement.close') }}"></button>
                    </div>
                    <div class="modal-body">
                        ...
                        <label for="add_new_role" class="form-label">{{ __('userManagement.role_name') }}</label>
                    ...
                    <div class="modal-body">
                        <label for="copy_role_name" class="form-label">{{ __('userManagement.clone_from') }}</label>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary"
                            data-bs-dismiss="modal">{{ __('userManagement.close') }}</button>
                        <button type="submit" class="btn btn-primary">{{ __('userManagement.save_role') }}</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="editRolesForm">
                    ...
                    <div class="modal-header">
                        <h5 class="modal-title" id="editModalLabel">{{ __('userManagement.edit_role') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                            aria-label="{{ __('userManagement.close') }}"></button>
                    </div>
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="edit_roles_name"
                                class="form-label">{{ __('userManagement.role_name') }}</label>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary"
                            id="editSaveBtn">{{ __('userManagement.save_changes') }}</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    @push('js')
        ...
        <script>
                ...
                $('.categories-show-table').DataTable({
                    ...
                    ajax: {
                        ...
                        error: function(xhr, error, code) {
                            Swal.fire("{{ __('userManagement.failed') }}",
                                "{{ __('userManagement.could_not_load_data') }}: " + error, 'error');
                        }
                    },
                    columns: [{
                            data: 'DT_RowIndex',
            ...
            $('#addRolesForm').on('submit', function(e) {
                ...
                btn.prop('disabled', true).text("{!! __('userManagement.saving') !!}");
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            ...
                            Swal.fire({
                                icon: 'success',
                                title: "{!! __('userManagement.success') !!}!",
                    ...
                    complete: function() {
                        btn.prop('disabled', false).text("{!! __('userManagement.saving') !!}");
                    }
                });
            });
            ...
            $('#editRolesForm').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    complete: function() {
                        btn.prop('disabled', false).text("{!! __('userManagement.save_changes') !!}");
                    }
                });
            });
