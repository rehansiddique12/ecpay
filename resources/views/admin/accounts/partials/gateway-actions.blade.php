<a href="#" class="edit-method-btn" data-bs-toggle="modal" data-bs-target="#editMethodModal"
    ...
    data-min_amount="{{ getAmount($gateway->min_amount) }}" data-max_amount="{{ getAmount($gateway->max_amount) }}"
    data-min_withdrawal="{{ getAmount($gateway->min_withdrawal_amount) }}"
    data-max_withdrawal="{{ getAmount($gateway->max_withdrawal_amount) }}"
    data-percentage_charge="{{ getAmount($gateway->percentage_charge) }}"
    data-fixed_charge="{{ getAmount($gateway->fixed_charge) }}" data-note="{{ $gateway->note }}"
    data-status="{{ $gateway->status }}" data-image="{{ getFile(config('location.gateway.path') . $gateway->image) }}"
...
<script>
    ...
    //             $.ajax({
    //                 url: '{{ route("admin.accounts.payment.methods.deactivate") }}', // Your delete route
    //                 method: 'DELETE',
    //                 data: {
    //                     _token: '{{ csrf_token() }}',
    //                     code: roleId
