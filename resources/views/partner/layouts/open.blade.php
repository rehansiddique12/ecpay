    ...
    {{-- <script>
        ...
        let pushNotificationArea = new Vue({
            ...
            methods: {
                getNotifications() {
                    let app = this;
                    axios.get("{{ route('partner.push.notification.show') }}")
                        .then(function(res) {
                            app.items = res.data;
                        })
                },
                readAt(id, link) {
                    let app = this;
                    let url = "{{ route('partner.push.notification.readAt', 0) }}";
                    url = url.replace(/.$/, id);
                    axios.get(url)
                        .then(function(res) {
                            if (res.status) {
                ...
                readAll() {
                    let app = this;
                    let url = "{{ route('partner.push.notification.readAll') }}";
                    axios.get(url)
                        .then(function(res) {
                            if (res.status) {
                                app.items = [];
                ...
                pushNewItem() {
                    ...
                    let channel = pusher.subscribe('admin-notification.' + "{{ Auth::guard('partner')->id() }}");
