            ...
            <div class="table-responsive">
                <table class="categories-show-table table table-hover table-striped table-bordered">
                    ...
                    @forelse($settlementsByDate as $key => $settlement)
                        <tr>
                            <td onclick="openmodel('{{ $settlement->settlement_date }}', '{{ $gateway }}', 'All')"> {{ $settlement->settlement_date }}</td>
                            <td onclick="openmodel('{{ $settlement->settlement_date }}', '{{ $gateway }}', 'Pending')"> {{ $settlement->pending_count }}</td>
                            <td onclick="openmodel('{{ $settlement->settlement_date }}', '{{ $gateway }}', 'Pending')"> {{ $settlement->pending_amount }}</td>
                            <td onclick="openmodel('{{ $settlement->settlement_date }}', '{{ $gateway }}', 'Approved')"> {{ $settlement->complete_count }}</td>
                            <td onclick="openmodel('{{ $settlement->settlement_date }}', '{{ $gateway }}', 'Approved')"> {{ $settlement->complete_amount }}</td>
                            <td onclick="openmodel('{{ $settlement->settlement_date }}', '{{ $gateway }}', 'All')"> {{ $settlement->settlement_count }}</td>
                            <td onclick="openmodel('{{ $settlement->settlement_date }}', '{{ $gateway }}', 'All')"> {{ $settlement->total_amount }}</td>
                        </tr>
...
function openmodel(date, gateway, status) {
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      var thead = $('<thead class="thead-dark"><tr><th>Source</th><th>Source Name</th><th>Account No</th><th>Amount</th><th>Charges</th><th>Net Amount</th><th>Status</th><th>Created At</th><th>Updated At</th></tr></thead>');
      ...
      for (var i = 0; i < response.length; i++) {
        ...
        if (response[i].status == 2) {
            ...
            statusBadge = '<span class="badge badge-light"><i class="fa fa-circle text-success success font-12"></i> Approved</span>';
        } else {
        ...
        var updatedAt = new Date(response[i].updated_at).toLocaleString('en-GB', { day: 'numeric', month: 'numeric', year: 'numeric', hour: 'numeric', minute: 'numeric', hour12: true });
