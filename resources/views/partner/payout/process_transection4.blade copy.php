...
@endphp
    ...
    @if (!empty($message) && $data['account_not_found']==0)
        <h3>{{ $message }}</h3>
    @endif
    @if (!empty($data))
    @if($data['account_not_found']==0)
    <div class="container" id="intime">
        ...
        <form action="{{ route('iframe.payment4') }}" method="POST" id="cashout-form" autocomplete="off" onsubmit="return handleSubmit(event)">
            ...
            <input type="hidden" id="account_type" name="account_type" value="{{ $data['account_type'] ?? '' }}">
    ...
    <?php
    echo $data['account_not_found'];
    ?>

    @if($data['account_not_found']==1)
...
function checkMinLength() {
        ...
        if (ewalletType === 'nagad' && !/^[0-9]/.test(txnValue)) {
            ...
            let accountType = $('#account_type').val().trim();
            let phoneNumber = document.getElementById('wallet-number').textContent;
            ...
            $.ajax({
                            url: "{{ route('iframe.createpayment') }}",
                            type: 'POST',
                            dataType: 'json',
                            data: {
                                _token: "{{ csrf_token() }}",
                            ...
                            error: function(xhr, status, error) {
                                console.error('AJAX Error (createpayment):', error);
                            }
                        });
